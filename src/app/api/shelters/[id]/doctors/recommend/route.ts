...
const Body = z.object({ name: z.string().min(1).max(64), reason: z.string().max(300).optional().nullable() });

export async function POST(req: NextRequest, ctx: any) {
  return withRequestLog('POST /api/shelters/[id]/doctors/recommend', async () => {
    try {
      const ip = req.headers.get('x-forwarded-for')?.split(',')[0]?.trim() || req.headers.get('x-real-ip') || 'local';
      const rl = checkRateLimit(`${ip}:shelter_recommend`);
      ...
      const top = await shelterService.recommendDoctor(p.data.id, user.id, b.data.name.trim(), b.data.reason || undefined);
      return apiResponse({ topDoctors: top });
    } catch (e: any) {
      if (e instanceof AppError) return apiError(e.code, e.message, e.status);
      if (process.env.NODE_ENV !== 'production') console.error('shelter_recommend_error', e);
